var projektURL = "http://localhost:8080/i_advise_zeiterfassung/rest/projekt";
var buchungURL = "http://localhost:8080/i_advise_zeiterfassung/rest/buchung";
...
function findAlleProjekte() {
	console.log('findAlleProjekte');
	$.ajax({
		type : 'GET',
		url : projektURL,
...
function renderSelect(data){
	projektliste = data == null ? [] : (data instanceof Array ? data
			: [ data ]);
	console.log("Projektliste: "+projektliste[0].mitid.mitid);
	$.each(projektliste, function(index, proj){
		console.log(proj.projektname);
		$("<option></option>", {value: proj.projektname,text: proj.projektname }).appendTo('.select-group');
	});
}

function getProjektid(projektname){
	var prjid;
	$.each(projektliste, function(index, proj){
		if(proj.projektname == projektname)
			prjid = proj.prjid;
...
function addBuchung() {
	...
	$.ajax({
		...
		success : function(data, textStatus, jqXHR) {
			alert('Buchung created successfully ');
						
		},
		error : function(jqXHR, textStatus, errorThrown) {
			alert('addBuchung error: ' + textStatus);
		}
	});
}
...
function formToJSON() {
	...
	var projektname = $('.select-group').val();
	var mitid = projektliste[0].mitid.mitid
	projektid = getProjektid(projektname);
	return JSON.stringify({
		"prjid" : projektid,
		"buID" : null,
		"mitid" : mitid,
		"stichtag" : $('#stichtag').val(),
		"budgetstunden" : $('#budgetstunden').val()
	});
